...
const Suppliers = () => {
    ...
    const createMutation = useMutation({
        mutationFn: async (data: any) => {
            await client.post('/suppliers', data);
        },
        onSuccess: () => {
        ...
        onError: (err: any) => {
            toast.error(err.response?.data?.message || 'Failed to onboard supplier');
        }
    });
            ...
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <p className="text-text-secondary font-medium flex items-center gap-2 italic">
                        <Globe size={14} className="text-accent" /> Managing relationships across {suppliers?.length || 0} active global suppliers
                    </p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {isLoading ? (
                    ...
                    <motion.div
                        ...
                        whileHover={{ y: -5 }}
                        key={supplier._id}
                        className={`glass-card p-8 border-none bg-gradient-to-br from-secondary/50 to-white/[0.02] shadow-2xl relative overflow-hidden group transition-all ${supplier.isActive === false ? 'opacity-60 saturate-50' : ''}`}
                    >
                        ...
                        <div className="flex items-center gap-5 mb-8">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                    <h3 className="text-xl font-black text-white truncate group-hover:text-accent transition-colors">{supplier.name}</h3>
                                    <div className={`w-2 h-2 rounded-full ${supplier.isActive !== false ? 'bg-success animate-pulse' : 'bg-danger'}`} />
                                </div>
                                <p className={`text-[10px] font-black uppercase tracking-widest mt-1 ${supplier.isActive !== false ? 'text-success/70' : 'text-danger/70'}`}>
                                    {supplier.isActive !== false ? 'Operational' : 'Deactivated'}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="group/info cursor-pointer">
                                <div className="flex items-center gap-4 text-sm font-bold text-text-secondary group-hover/info:text-white transition-colors">
                                    ...
                                    <span className="truncate">{supplier.address || 'Global Distribution Center'}</span>
                                </div>
                            </div>
                            {supplier.pricing && Object.keys(supplier.pricing).length > 0 && (
                                <div className="p-3 rounded-xl bg-white/5 border border-white/5">
                                    <p className="text-[10px] font-black text-accent uppercase tracking-widest mb-2">Price Sheet Snapshot</p>
                                    <div className="space-y-1">
                                        {Object.entries(supplier.pricing).slice(0, 3).map(([sku, price]: any) => (
                        ...
                        <div className="mt-10 pt-8 border-t border-white/5 flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-sm font-bold text-white">{supplier.contactName || 'Executive Lead'}</p>
                            </div>
                            <button
                                onClick={() => {
